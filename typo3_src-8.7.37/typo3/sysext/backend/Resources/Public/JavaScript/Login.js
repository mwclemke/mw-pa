 ...
 */
define(['jquery', 'TYPO3/CMS/Backend/Storage', 'TYPO3/CMS/Backend/jquery.clearable', 'bootstrap'], function($, Storage) {
  'use strict';

  /**
   *
   * @type {{options: {loginForm: string, loginUrlWrapper: string, interfaceField: string, useridentField: string, submitButton: string, error: string, errorNoCookies: string, formFields: string, submitHandler: null}, ready: boolean}}
  ...
  BackendLogin.checkDocumentReferrerSupport = function() {
    var referrerRefreshed = Storage.Client.get('referrerRefresh') === '1';
    var loginUrlWrapper = document.getElementById(BackendLogin.options.loginUrlWrapper);
    ...
    if (typeof document.referrer === 'string' && document.referrer !== '') {
      if (referrerRefreshed) {
        Storage.Client.unset('referrerRefresh');
      }
      return;
    }
    if (referrerRefreshed) {
      Storage.Client.unset('referrerRefresh');
      $(BackendLogin.options.errorNoReferrer).removeClass('hidden');
    } else {
      BackendLogin.ready = false;

      Storage.Client.set('referrerRefresh', '1');
  ...
  $(function() {
    ...
    BackendLogin.checkDocumentReferrerSupport();
